...
BlockController.prototype.checkBlockHash = function(req, res, next) {
  ...
  if (hash.length < 64 || !isHexadecimal(hash)) {
    this.node.services.galactrumd.getBlockHeader(parseInt(hash), function(err, info) {
      if (err) {
...
BlockController.prototype.blockHeader = function(req, res, next) {
	...
	self.node.services.galactrumd.getBlockHeader(hash, function(err, info) {
		if (err) {
			return self.common.handleErrors(err, res);
		}
...
BlockController.prototype.blockHeaders = function(req, res, next) {
	...
	if(blockIdentifier.length===64){
		...
		var result = self.node.services.galactrumd.getBlockHeaders(hash, cb, nbOfBlockToFetch);
	}else{
		var height = blockIdentifier;
		var result = self.node.services.galactrumd.getBlockHeaders(height, cb, nbOfBlockToFetch);
	}
...
debugger;
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.galactrumd.height - blockCached.height + 1;
...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix orecore to give back null instead of null hash
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.galactrumd.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.galactrumd.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }

      var br = new orecore.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in orecore-node.
      var header = orecore.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [orecore.Transaction().fromBufferReader(br)];

      self.node.services.galactrumd.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.galactrumd.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  self.node.services.galactrumd.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
...
BlockController.prototype.getPreviousBlock = function(prevHash, cb) {
  ...
  if(prevHash===null) cb(null,null);
      ...
      self.node.getBlock(prevHash, function (err, block) {
          ...
          self.node.services.galactrumd.getBlockHeader(prevHash, function (err, info) {
              if (err) {
...
BlockController.prototype.getBlockReward = function(prevHash, cb) {
  var self = this;
  var nSubsidyHalvingInterval = 1314000;
  var nBudgetPaymentsStartBlock = 197100;
  var nSubsidyBase;
  ...
  self.getPreviousBlock(prevHash, function(err, info) {
    ...
    if(info===null){
        //Genesis reward is 972,000.
        cb(null, parseFloat("972000".toString(10)).toFixed(8));
    }
    else{
